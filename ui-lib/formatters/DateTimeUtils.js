sap.ui.define([
    ...
    var oLocale = new sap.ui.core.Locale(sap.ui.getCore().getConfiguration().getLocale().toLocaleString())
    var oDateSetting = {
        format: "yMMMd"
    };
    var oDateTimeSetting = {
        format: "yMMMdHms"
    };
    var oDateIntervalSetting = {
        format: "yMMMd",
        interval: true
    };
    var oTimeSetting = {
    ...
    var regDate = /^(\d{4}-\d{2}-\d{2})(\s(\d\d):(\d\d):(\d\d))?$/i;
    var regISODate = /^\d{4}(-\d\d(-\d\d(T\d\d:\d\d(:\d\d)?(\.\d+)?(([+-]\d\d:\d\d)|Z)?)?)?)?$/i;

    var oDateInstance = DateFormat.getDateInstance(oDateSetting, oLocale);
    var oDateTimeInstance = DateFormat.getDateTimeInstance(oDateTimeSetting, oLocale);
    var oDateIntervalInstance = DateFormat.getDateInstance(oDateIntervalSetting, oLocale);
    var oTimeInstance = DateFormat.getDateInstance(oTimeSetting, oLocale);
    ...
    return {
        ...
        formatDateTime: function(vDateTime) {
            var dDate = parseDateTime(vDateTime);
        ...
        dmcFormatDateInterval: function (sDateStart, sDateEnd) {
            ...
            const oLocaleInternal = _returnCurrentLocale();
            const odmcDateIntervalInstance = DateFormat.getDateInstance({interval:true}, oLocaleInternal);
